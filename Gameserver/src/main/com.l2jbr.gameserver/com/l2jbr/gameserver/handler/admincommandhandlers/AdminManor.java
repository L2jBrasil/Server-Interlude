    ...
    public boolean useAdminCommand(String command, L2PcInstance activeChar) {
        ...
        if (command.equals("admin_manor")) {
            ...
            if (castleId > 0) {
                ...
                castle.setCropProcure(new LinkedList<>(), CastleManorManager.PERIOD_CURRENT);
                castle.setCropProcure(new LinkedList<>(), CastleManorManager.PERIOD_NEXT);
                castle.setSeedProduction(new LinkedList<>(), CastleManorManager.PERIOD_CURRENT);
                castle.setSeedProduction(new LinkedList<>() {
                ...
                for (Castle castle : CastleManager.getInstance().getCastles()) {
                    ...
                    castle.setCropProcure(new LinkedList<>(), CastleManorManager.PERIOD_NEXT);
                    castle.setSeedProduction(new LinkedList<>(), CastleManorManager.PERIOD_CURRENT);
                    castle.setSeedProduction(new LinkedList<>(), CastleManorManager.PERIOD_NEXT);
                    if (Config.ALT_MANOR_SAVE_ALL_ACTIONS) {
                        castle.saveCropData();
                        castle.saveSeedData();
                    }
