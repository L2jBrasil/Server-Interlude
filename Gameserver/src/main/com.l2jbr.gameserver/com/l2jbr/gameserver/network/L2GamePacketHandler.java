...
public final class L2GamePacketHandler implements PacketHandler<L2GameClient>, ClientFactory<L2GameClient>, PacketExecutor<L2GameClient>
{
            ...
            case IN_GAME:
                switch (opcode)  {
                    ...
                    case 0xd0:
                        int id2;
                        if (data.available() >= 2) {
                            id2 = toUnsignedInt(data.getShort());
                        }
                        else
                        {
                        ...
                        switch (id2)
                        {
                            ...
                            case 0x30:
                                ...
                                break;
                            default:
                                printDebugDoubleOpcode(opcode, id2, data, state, client);
                                break;
                        }
                        break;
                    /*
	...
	private void printDebug(int opcode, DataWrapper data, GameClientState state, L2GameClient client) {
		...
		_log.warn(Util.printData(data.expose()));
	}
	
	private void printDebugDoubleOpcode(int opcode, int id2, DataWrapper data, GameClientState state, L2GameClient client) {
		_log.warn("Unknown Packet: {}:{} on State: {} Cliente {}", Integer.toHexString(opcode), Integer.toHexString(id2), state.name(), client.toString());
		_log.warn(Util.printData(data.expose()));
	}
