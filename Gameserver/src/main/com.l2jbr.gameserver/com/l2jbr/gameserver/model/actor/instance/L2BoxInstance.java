...
public class L2BoxInstance extends L2NpcInstance {
    ...
    // private static Logger _log = LoggerFactory.getLogger(L2BoxInstance.class.getName());
    ...
    private void showDepositWindow(L2PcInstance player, String command) {
        ...
        NpcHtmlMessage html = new NpcHtmlMessage(getObjectId());
        int nitems = 0;
        Set<L2BoxItem> _items = new LinkedHashSet<>();
    ...
    private Set<L2BoxItem> getItems(String drawer) {
        Set<L2BoxItem> it = new LinkedHashSet<>();
        java.sql.Connection con = null;
        try {
            con = L2DatabaseFactory.getInstance().getConnection();
     ...
     * newItem.setEnchantLevel(item.getEnchantLevel()); putItemInBox(player, drawername, newItem); } catch (Exception e) { _log.debug("putInBox "+command+" failed: "+e); } } } private void putItemInBox(L2PcInstance player, String drawer, L2ItemInstance item) { String charname = player.getName();
     ...
     * statement.setString(5, item.getItem().getName()); statement.setInt(6, item.getCount()); statement.setInt(7, item.getEnchantLevel()); statement.execute(); statement.close(); } else { PreparedStatement statement = con.prepareStatement("UPDATE boxes SET count=? WHERE id=?"); statement.setInt(1,
     * foundCount + item.getCount()); statement.setInt(2, foundId); statement.execute(); statement.close(); } } catch (Exception e) { _log.info("could not store item to box "+getSpawn().getId()+"-"+drawer+" for char "+charname); } finally { try { con.close(); } catch (Exception e) {} }
     */
    }
     ...
     * item.setEnchantLevel(bi.enchant); player.getInventory().addItem("Box", item, player, this); } } catch (Exception e) { _log.debug("takeOutBox "+command+" failed: "+e); } } } private L2BoxItem takeItemOutBox(L2PcInstance player, String drawer, L2ItemInstance item) { String charname =
     ...
     * _log.info("could not delete/update item, box "+getSpawn().getId()+"-"+drawer+" for char "+charname+": "+e); } finally { try { con.close(); } catch (Exception e) {} } return bi;
     */
    }
}
